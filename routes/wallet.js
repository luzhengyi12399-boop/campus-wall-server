...
router.post('/recharge', authMiddleware, (req, res) => {
  ...
  db.prepare('INSERT INTO wallet_history (userId, type, amount, balance, description, createdAt) VALUES (?, ?, ?, (SELECT wallet FROM users WHERE id = ?), ?, ?)')
    .run(req.userId, 'recharge', amount, req.userId, 'Wallet recharge', Date.now());
